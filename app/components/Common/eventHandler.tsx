...
interface AddHoverProps {
  type: string,
  className: string,
  onClick: any,
}

interface AddHoverState {
  hover: boolean,
  touch: boolean,
}

export class AddHover extends React.Component<AddHoverProps, AddHoverState> {
   ...
   */
  updateHover (hoverOn: boolean, isTouch: boolean) {
    if (hoverOn) {
  ...
  render () {
    let className: string = this.props.className.replace(/(^| )hover( |$)/g, '') + (this.state.hover ? ' hover' : '')
    let Container: string = this.props.type || 'div'
