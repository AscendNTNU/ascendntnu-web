...
export class JoinPage extends Component {
  ...
  render() {
    let positions = {
      ...
      'no': [ <
        ...
        Det er Hardware gruppen som designer og bygger selve dronene til Ascend, og her kan du jobbe med litt av hvert.Gruppen er en krysning mellom mekanisk ingeniørvitenskap, produktdesign og elektronikk.Dette involverer å kunne designe og produsere delene til dronen, designe det elektriske systemet ombord på dronen og samarbeide med de andre gruppene
        for å integrere sensorikk og datamaskinene vi har ombord!Her trenger vi mange forskjellige talenter.Du vil bli kurset i slik teknologi om du ikke har erfaring med det fra før. < br / >
        <
        ...
        Som teknisk gruppeleder er det ditt ansvar å holde oversikt over det tekniske arbeidet som må gjøres på gruppen din, sørge
        for at alle har det de trenger, sørge
        for at gruppen kommuniserer med de andre tekniske gruppene og at alle i Ascend er oppdatert.Du vil fordele arbeidsoppgaver samtidig som du selv blir å gjøre like mye hands - on teknisk arbeid som gruppen din.Du vil også bli kurset i ledelse om du ikke har erfaring med det fra før. < br / >
        <
        ...
        Som teknisk gruppeleder er det ditt ansvar å holde oversikt over det tekniske arbeidet som må gjøres på gruppen din, sørge
        for at alle har det de trenger, sørge
        for at gruppen kommuniserer med de andre tekniske gruppene og at alle i Ascend er oppdatert.Du vil fordele arbeidsoppgaver samtidig som du selv blir å gjøre like mye hands - on teknisk arbeid som gruppen din.Du vil også bli kurset i ledelse om du ikke har erfaring med dette fra før. < br / >
        <
        ...
        p id = 'sponsorkontakt' >
        Har du lyst til å være Ascend NTNU sin representant i næringslivet ? Vi i Ascend er helt avhengig av våre sponsorer
        for å kunne gjøre det arbeidet vi gjør.Som sponsorkontakt i Ascend har du som hovedoppgave å skape mest mulig verdiskapning
        for våre sponsorer.Du vil jobbe med å finne nye sponsorer, forhandle kontrakter og oppretholde god kommunikasjon med våre eksisternede sponsorer.Du vil også jobbe tett sammen med hele kommunikasjonsgruppen
        ...
        utad.Som grafisk designer vil du lage alt fra markedsmateriell til profileringartikler og du vil skape en ordentlig grafisk utforming i alt vi gjør.Du vil jobbe tett sammen med hele kommunikasjons gruppen, blant annet ved å bidra til vår blog, lage vårt aller første magasin og bidra til utformingen av stand.Det er et pluss om du vil hjelpe til med videoer, 3 D visualisering eller motion design, men det er ikke et krav. < br / >
        <
        i className = 'key-words' > Nøkkelord: Design, t - skjorter, genser, jakker, visittkort, medaljer, roll - ups, flyers, postere, webdesign, magasin design, redaktør, video, motion design, 3 D visualisering. < /i> < /
        p > <
        h3 > Event Manager < /h3> <
        p id = 'event-manager' >
        Som Event Manager får du muligheten til å arrangere events som ikke bare når ut til alle studentene på NTNU, men i tillegg både til presse og næringsliv i hele Norge.Du får ansvaret
        for å arrangere ulike stands og fremføringer på ulike arrangementer, både internt på NTNU og utad mot næringslivet.Du vil også få ansvaret
        for organiseringen av våre opptaksperioder i tillegg til å arrangere egne arrangementer som
        for eksempel robotikk dag og live - demo / unveiling.Som Event Manager vil du jobbe tett sammen med hele kommunikasjons gruppen, spesielt vår sponsorkontakt
        for å fremme Ascend til potensielle nye medlemmer og sponsorer.Du vil også være med på å planlegge den årlige turen vår til USA. < br / >
        <
        ...
        h3 > Webutvikler < /h3> <
        p id = 'webutvikler' >
        Har du lyst å utvikle en full - stack, skalerbar og grafisk imponerende webside ? Som webutvikler vil du ha ansvar
        for å videreutvikle vår hjemmeside ascendntnu.no.Det blir også din oppgave å vise frem hvem vi er, våre mål og hvilke utfordringer vi møter på en interaktiv og spennende måte, kanskje ved å lage en liten spill app ? Du vil jobbe tett sammen med hele kommunikasjons gruppen
        for å videreutvikle Ascends "image"
        utad.Du kan også ta på deg ansvaret med system administrasjon, det vil si at du blant annet får bygge og vedlikeholde våre byggservere og sørge
        for at alt går så automatisk og knirkefritt som mulig.Du vil bli kurset i webutvikling(og noe sysadmin) om du ikke har erfaring med det fra før. < br / >
        <
        ...
        key = '2' >
        <
        p > Bli med i styret og bestem fremtiden til Ascend, eller lag teknologi som verden ikke har sett maken til som teknisk gruppeleder. < /p> <
        ul className = 'anchor-list' >
        <
        li onClick = {
        ...
        <
        li onClick = {
          () => this.setAnchor('Marked og kommunikasjons leder')
        } > < a > Marked og kommunikasjons leder < /a></li >
        <
        li onClick = {
        ...
        <
        p > Prosjektleder er styrets leder og jobber med å videreutvikle Ascend i ønsket retning basert på Ascends visjoner og mål.Som prosjektleder legger man i stor grad føringer
        for hvordan hele organisasjonen skal drives og hvordan arbeidet skal gjennomføres.Stillingen krever at man er proaktiv og ser forbedringsområder samt legger til rette
        for god arbeidskultur.Prosjektleder jobber tett opp mot alle Ascends undergrupper og koordinerer mye av arbeidet som skjer mellom gruppene på et overordnet plan.Sammen med marked og kommunikasjonsgruppen jobber prosjektleder
        for å fremme Ascends omdømme og jobbe med sponsorer.Arbeidsoppgavene man har vil variere fra uke til uke og gir muligheter til å jobbe med de fleste aspekter i prosjektet.Faste oppgaver inkluderer å lede styremøter og allmøter, og ansvar
        for samarbeid med hovedsponsor. < /p> <
        p > Som nestleder har man mange spennende oppgaver.Først og fremst er man hovedansvarlig
        for rekruttering av nytt styre, nye gruppemedlemmer og gruppeledere.Man skal også bistå prosjektleder med planlegging, oppfølging og målsetting.Videre blir du en tillitsperson
        for gruppemedlemmene og sosialt ansvarlig. < /p> <
        p > Prosjektleder og nestleder jobber tett sammen og fordeler arbeidsoppgaver mellom seg.Dette samarbeidet gir en stor fleksibilitet når det kommer til arbeidsoppgaver, og lar begge to tilpasse ansvarsområder slik at man kan jobbe med det man har størst interesse
        for. < /p> <
        ...
        ul > <
        p > Har du spørsmål om denne stillingen kan du kontakte Simen Sandbu(Prosjektleder) på < a href = {
          `mailto:${this.emails[0]}ascendntnu.no`
        } > {
          this.emails[0]
        }
        ascendntnu.no < /a>, eller Erling Solberg (Nestleder) på <a href={`mailto:${this.emails[1]}ascendntnu.no`}>{ this.emails[1] }ascendntnu.no</a > . < /p> < /
        SubSubSection > <
        ...
        p > Som nestleder vil du også ha ansvaret
        ...
        for å planlegge, kordinere og dokumentere testing av systemer. < /p> <
        p > < b > Vi ser etter < /b></p >
        <
        ul >
        <
        li > Du som har gode sammarbeidsevner og et engasjement rundt ledelse. < /li> <
        li > Du som har stor teknisk interesse
        for robotikk og gjerne erfaring fra c++eller ROS(Robot Operating System). < /li> <
        ...
        ul > <
        p > Har du spørsmål knyttet til disse stillingene kan du kontakte Kevin Kaldvansvik på < a href = {
        ...
        p > Din oppgave vil hovedsaklig være å forsikre at styret har oversiktlig regnskap, kontobalanse og budsjetter tilgjengelig i forbindelse med avgjørelser
        ...
        for økonomi, regnskapsføring og budsjettplanlegging. < /li> <
        li > Du som har kunnskap om, eller interesse
        for å lære om føring av økonomi i organisasjoner. < /li> <
        li > Du som har ryddige og oversiktlige arbeidsmetoder. < /li> < /
        ul > <
        p > Har du spørsmål om denne stillingen kan du kontakte Truls Olsen på < a href = {
        ...
        p > Som gruppeleder vil du lede en teknisk gruppe på 4 - 6 personer, ha ansvaret
        for å planlegge arbeid og sørge
        for at arbeid blir gjennomført.Du vil også jobbe tett sammen med tekniske ledere og andre gruppeledere. < /p> <
        SubSubSection titleText = 'Gruppeleder i Perception'
        id = 'Gruppeleder i Perception' >
        <
        p > I perception gruppen jobber vi med moderne og avanserte algoritmer innen datasyn og visuell databehandling.Her kan du få mulighet til å prøve deg på cutting edge "deep learning"
        ...
        for å holde gruppen din motivert og kreativ, ha oversikt på det tekniske arbeidet i din gruppe og ha god kontakt med andre tekniske gruppeledere. < /p> <
        ...
        p > Har du spørsmål om denne stillingen kan du kontakte Ulrich Isacsen på < a href = {
        ...
        <
        i className = 'key-words' > Stikkord: Reguleringsteknikk, systemutvikling, C / C++, ROS, Navigasjon, Path Planning, Obstacle Avoidance, Tilstandsmaskiner, Git < /i> <
        p > Har du spørsmål om denne stillingen kan du kontakte Håvard Mellbye på < a href = {
        ...
        for det mekaniske og elektriske systemet til dronene.Dette innebærer at du får innsikt i alt fra CFD og styrkeanalyser til kretskortdesign og produksjon, i tillegg til å designe dronen selv.Som leder må du koordinere med de andre tekniske gruppelederne i Ascend
        for å integrere sensorikk og datamaskiner ombord.Det er en fordel om du har tidligere erfaring med RC - utstyr eller stor interesse
        for det. < /p> <
        i className = 'key-words' > Stikkord: CAD, 3 D - print, Styrke og CFD analyser, Krets design, droneflyving, dronebygging og produsering av diverse komponenter. < /i> <
        p > Har du spørsmål om denne stillingen kan du kontakte Filip Lolland på < a href = {
        ...
        I AI gruppen jobber du med den overordnede kunstige intelligensen(AI) til dronen.Det er gruppen sitt ansvar å produsere en AI som kan analysere informasjonen om verden rundt seg og gjennom dette skal den kunne ta beslutninger på egen hånd og gjerne det smarteste valget.
        ...
        teknologi til implementering og testing av algoritmen i praksis.Her får du mulighet til å jobbe med et bredt spekter av AI algoritmer samt få erfaring i å ta noe veldig teoretisk og få det til å funke i den virkelige verden.
        ...
        i className = 'key-words' > Stikkord: Kunstig Intelligens Programmering, Maskinlæring, Deep Learning, Reinforcement Learning, Algoritmer og Datastrukturer, Statistikk. < /i> <
        p > Har du spørsmål om denne stillingen kan du kontakte Rasmus Munter på < a href = {
    ...
    let contents = {
      ...
      'no': [ <
          ...
          } > < b > Søknadsfrist 15. april kl .23: 59 < /b></p >
          <
      ...
      <
      li > < strong > Kombiner med graden din. < /strong> Flere av medlemmene våre skriver prosjekt/master - oppgave relatert til Ascend.I tillegg finnes det en åpning
      for å drive med Ascend i Eksperter i Team. < /li> <
      ...
      <
      li > < strong > Kreativ,
      fleksibel og innovativ kultur < /strong> Vi ønsker å holde en flytende arbeidskultur i Ascend slik at ditt bidrag til organisasjonen kan også involvere oppgaver utenfor stillingsbeskrivelsen som vil bli inspirert av dine interesser og erfaringer!</li >
      <
      /ul> < /
      SubSection > <
      SubSection titleText = 'Konkurransen' >
      <
      p > International Aerial robotics competition(IARC) er en årlig konkurranse som avholdes i USA og Kina med mål om å presse grensene
      for hva man får til med autonome droner.Dette gjør de ved å presentere meningsfulle og nyttige oppdrag som er umulige å løse på det tidspunktet de lanseres.Ideen er at når den oppførselen som etterspørres til slutt demonstres i konkurransen,
      vil konkurransen ha bidratt til en meningsfull utvikling innenfor autonom robotikk.En utvikling som fortrinnsvis er til fordel
      for verden. < /p> <
      p > < a href = 'https://ascendntnu.no/blog/2017-07-20-the-competition'
  ...
  let about = {
    ...
    'no': ( <
      ...
      for Teknisk Kybernetikk sin lab på taket av blokk A i El - bygget på Gløshaugen, kjent som taklabben.Taklabben er et arbeidslokale
      for alle i Ascend som har flere kraftige datamaskiner, alt utstyr du måtte trenge og en egen intern kiosk.På den måten blir veien fra utvikling til testing kort. < /p> < /
      SubSection > <
      SubSection titleText = 'Hvem vi jobber med' >
      <
      p > Vi har god kontakt med Institutt
      for Teknisk Kybernetikk(ITK), Institutt
      for Datateknikk og Informasjonsvitenskap(IDI) og alle våre sponsorer.Gjennom instituttene får vi tilgang til diverse labber, utstyr og kompetanse.I tillegg skriver flere av våre medlemmer prosjekt / master knyttet til Ascend
      for IDI og ITK.En oversikt over våre sponsorer finner du nederst på hovedsiden vår. < /p> < /
      SubSection > <
