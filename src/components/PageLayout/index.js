...
export class Title extends Component {
  render() {
    return <h1 className="section-title">{this.props.children}</h1>
  }
}

export class SubTitle extends Component {
  render() {
    return <h2 className="sub-section-title">{this.props.children}</h2>
  }
}

export class SubSubTitle extends Component {
  render() {
    return <h3 className="sub-sub-section-title">{this.props.children}</h3>
  }
}
...
export class Section extends Component {
  ...
  render() {
    if (this.props.titleText) {
      this.title = <Title>{this.props.titleText}</Title>
    }
...
export class SubSection extends Component {
  ...
  render() {
    if (this.props.titleText) {
      this.title = <SubTitle>{this.props.titleText}</SubTitle>
    }
...
export class SubSubSection extends Component {
  ...
  render() {
    if (this.props.titleText) {
      this.title = <SubSubTitle>{this.props.titleText}</SubSubTitle>
    }
...
export class ToTopButton extends Component {
  constructor(props) {
    ...
    document
      .querySelector('#app')
      .addEventListener('touchmove', this.scrollHandler.bind(this))
    document
      .querySelector('#app')
      .addEventListener('touchend', this.scrollHandler.bind(this))
  }
  ...
  scrollHandler(evt) {
    ...
    if ((app.scrollTop || document.body.scrollTop) > 100 && this.state.onTop) {
      ...
      (app.scrollTop || document.body.scrollTop) <= 100 &&
      !this.state.onTop
    ) {
      this.setState({
        onTop: true,
  ...
  clickHandler(evt) {
    ...
    let interval = setInterval(() => {
      app.scrollTop -=
        10 / (0.1 + Math.pow(app.scrollTop / startScroll - 0.5, 2))
      document.body.scrollTop -=
        10 / (0.1 + Math.pow(document.body.scrollTop / startScroll - 0.5, 2))
  ...
  render() {
    return (
      <AddHover
        type="div"
        className={'totop-button' + (this.state.onTop ? ' on-top' : '')}
